...
LDMA_TransferCfg_t trans1;

LDMA_Descriptor_t descr1;
/**************************************************************************//**
 * @brief LDMA Handler
 *****************************************************************************/
uint32_t flag;
...
void initLdma(void)
{
  ...
  // Transfers trigger off ADC single conversion complete
  trans1 = (LDMA_TransferCfg_t)LDMA_TRANSFER_CFG_PERIPHERAL(ldmaPeripheralSignal_ADC1_SINGLE);

  descr1 = (LDMA_Descriptor_t)LDMA_DESCRIPTOR_LINKREL_P2M_BYTE(
      &(ADC1->SINGLEDATA),  // source
      adcBuffer1,            // destination
      ADC_BUFFER_SIZE,      // data transfer size
      0);                   // link relative offset (links to self)

  descr1.xfer.ignoreSrec = true;       // ignores single requests to save energy
  descr1.xfer.size = ldmaCtrlSizeWord; // transfers words instead of bytes

  // Initialize transfer
  LDMA_StartTransfer(LDMA_CHANNEL, &trans, &descr);
  LDMA_StartTransfer(LDMA_CHANNEL1, &trans1, &descr1);
...
void initAdc(void)
{
  ...
  CMU->ADCCTRL = CMU_ADCCTRL_ADC0CLKSEL_AUXHFRCO|CMU_ADCCTRL_ADC1CLKSEL_AUXHFRCO;
  //CMU->ADCCTRL = CMU_ADCCTRL_ADC1CLKSEL_AUXHFRCO;
  ...
  initSingle.posSel = adcPosSelAPORT4XCH11;  //PE11
  ...
  initSingle.posSel = adcPosSelAPORT4YCH10;  //PE10
  ADC_Init(ADC1, &init);
